...
from OneloveMusic import YouTube, app
from OneloveMusic.core.call import Onelove
from OneloveMusic.misc import SUDOERS, db
...
from OneloveMusic.utils.decorators.language import languageCB
from OneloveMusic.utils.formatters import seconds_to_min
from OneloveMusic.utils.inline.play import panel_markup_1, stream_markup, telegram_markup
from OneloveMusic.utils.stream.autoclear import auto_clean
from OneloveMusic.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await CallbackQuery.answer()
        await Onelove.stop_stream(chat_id)
        ...
        await Onelove.mute_stream(chat_id)
        ...
        await Onelove.unmute_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Onelove.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Onelove.skip_stream(chat_id, link, video=status)
            except Exception:
            ...
            try:
                await Onelove.skip_stream(chat_id, file_path, video=status)
            except Exception:
                return await mystic.edit_text(_["call_9"])
            ...
            try:
                await Onelove.skip_stream(chat_id, videoid, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            button = telegram_markup(_, chat_id)
            run = await CallbackQuery.message.reply_photo(
            ...
            try:
                await Onelove.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(_["call_9"])
            if videoid == "telegram":
                button = telegram_markup(_, chat_id)
